...
std::optional<QString> BlockDeviceHelper::resolveDeviceRefToDevNode(const QString& deviceRef, QString* fsTypeOut)
{
    ...
    for (const BlockDevice& device : devices) {
        if (device.deviceId == deviceRef ||
            device.devNode == deviceRef ||
            device.uuid == deviceRef ||
            device.partuuid == deviceRef) {
            if (fsTypeOut) {
                *fsTypeOut = device.fsType;
            }
    ...
    if (deviceRef.startsWith(QStringLiteral("uuid:"))) {
        const QString uuid = deviceRef.mid(QStringLiteral("uuid:").size()).toLower();

        for (const BlockDevice& device : devices) {
            if (device.uuid.toLower() == uuid) {
                if (fsTypeOut) {
                    *fsTypeOut = device.fsType;
                }
    ...
    if (deviceRef.startsWith(QStringLiteral("partuuid:"))) {
        const QString partuuid = deviceRef.mid(QStringLiteral("partuuid:").size()).toLower();

        for (const BlockDevice& device : devices) {
            if (device.partuuid.toLower() == partuuid) {
                if (fsTypeOut) {
                    *fsTypeOut = device.fsType;
                }
